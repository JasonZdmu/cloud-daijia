...
public class DriverAccountServiceImpl extends ServiceImpl<DriverAccountMapper, DriverAccount> implements DriverAccountService {
    ...
    public Boolean transfer(TransferForm transferForm) {
        ...
        wrapper.eq(DriverAccountDetail::getTradeNo,transferForm.getTradeNo());  //通过tradeNo来进行去重
        Long count = driverAccountDetailMapper.selectCount(wrapper); //查询有没有记录
        if(count > 0) { //代表存在这个交易记录就不需要进行再次交易了
            return true;
        }
