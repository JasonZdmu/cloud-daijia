...
public class OrderInfoServiceImpl extends ServiceImpl<OrderInfoMapper, OrderInfo> implements OrderInfoService {
    ...
    public Long saveOrderInfo(OrderInfoForm orderInfoForm) {
        ...
        //订单号
        String orderNo = UUID.randomUUID().toString().replaceAll("-",""); //用UUID生成订单号
        orderInfo.setOrderNo(orderNo);
        //订单状态
        orderInfo.setStatus(OrderStatus.WAITING_ACCEPT.getStatus()); //状态设置为等待接单状态
        orderInfoMapper.insert(orderInfo); //将订单信息插入到订单数据表中
    ...
    public CurrentOrderInfoVo searchCustomerCurrentOrder(Long customerId) {
        ...
        wrapper.in(OrderInfo::getStatus,statusArray);  //相当于数据库的status in(一个范围)

        //获取最新一条记录
        wrapper.orderByDesc(OrderInfo::getId);  //通过id做降序排列
        wrapper.last(" limit 1"); //通过last,然后限制 一条
    ...
    public Boolean updateOrderCart(UpdateOrderCartForm updateOrderCartForm){
        ...
        //修改的条件wrapper
        LambdaQueryWrapper<OrderInfo> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(OrderInfo::getId,orderInfo.getId());
    ...
    public OrderRewardVo getOrderRewardFee(String orderNo) {
        ...
        OrderBill orderBill =
                orderBillMapper.selectOne(new LambdaQueryWrapper<OrderBill>()
                        ...
                        .select(OrderBill::getRewardFee));  //相当于两表联查
